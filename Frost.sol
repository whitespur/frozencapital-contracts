...
*/
contract Frost is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 12h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-6 FROST-WAVAX LP -> FROST pool
    uint256 public constant INITIAL_FROST_POOL_DISTRIBUTION = 140000 ether;
    ...
    uint256 public burnThreshold = 1.10e16;
    ...
    /**
     * @notice Constructs the FROST ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("FROST", "FROST") {
        // Mints 1 FROST to contract creator for initial pool setup
    ...
    function _getFrostPrice() internal view returns (uint256 _frostPrice) {
        try IOracle(frostOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Frost: failed to fetch FROST price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _frostPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_frostPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFrostOracle(address _frostOracle) public onlyOperatorOrTaxOffice {
        require(_frostOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints FROST to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FROST to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFrostPrice = _getFrostPrice();
            currentTaxRate = _updateTaxRate(currentFrostPrice);
            if (currentFrostPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_frostPool != address(0), "!_frostPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_frostPool, INITIAL_FROST_POOL_DISTRIBUTION);
    }
